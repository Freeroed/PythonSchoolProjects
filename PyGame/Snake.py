...
class snake(object):
    ...
    def checking(self):
        answer = True
        for i in range(2,len(self.segments)-1): #Берём не от начала так как в первые 4 элемента хвоста мы не сможем врезаться
...
def chek_dead(snake_head, win_w, win_h):
    ...
    elif snake_head.y+10>window_h: return False
    else: return True
...
while run:
    ...
    if (keys[pygame.K_DOWN] or keys[pygame.K_s]) and sn.speed_y==0 and sn.segments[0].y <= sn.segments[1].y and sn.segments[0].x != sn.segments[1].x :
        sn.speed_x=0
        sn.speed_y=20
    elif (keys[pygame.K_UP] or keys[pygame.K_w]) and sn.speed_y==0 and sn.segments[0].y >= sn.segments[1].y and sn.segments[0].x != sn.segments[1].x:
        sn.speed_x=0
        sn.speed_y=-20
    elif (keys[pygame.K_LEFT] or keys[pygame.K_a])and sn.speed_x==0 and sn.segments[0].x >= sn.segments[1].x and sn.segments[0].y != sn.segments[1].y:
        sn.speed_x=-20
        sn.speed_y=0
    elif (keys[pygame.K_RIGHT] or keys[pygame.K_d])and sn.speed_x==0 and sn.segments[0].x <= sn.segments[1].x and sn.segments[0].y != sn.segments[1].y:
        sn.speed_x=20
        sn.speed_y=0
    if (keys[pygame.K_g]):
        sn.add()
